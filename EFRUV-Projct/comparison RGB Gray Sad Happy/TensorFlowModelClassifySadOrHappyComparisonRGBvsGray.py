...
def transformImageGray(file):
    ...
    #store the grayscale information in a numpy array (width, height, color).
    picture = np.array([[np.array([pix[x,y]]) for y in range(height)] for x in range(width)], np.int32)
...
def tensorFlowModelRGB(trainingSet,trainingLabels,testingSetFiles,testingLabels,batchSize):
    ...
    for file in testingSetFiles:
        testingSetTmp.append(transformImageRGB(file))
    testingSet = np.asarray(testingSetTmp)
    del testingSetTmp[:]
    ...
    for step in range(0,len(trainingSet),batchSize):
        ...
        for image in trainingSet[batchBegin:batchEnd]:
            nextBatch.append(transformImageRGB(image))
...
def tensorFlowModelGray(trainingSet,trainingLabels,testingSetFiles,testingLabels,batchSize):
    """The actual tensorflow Model
    Here we initialise first all the variables, such as the numpy images, the labels, the learning rate, the weights, the bayes.
    We then initialize a convolutional layer and initialize ReLu and max pooling
    Then we reshape it for the processing
    ...
    testingSetTmp = []
    for file in testingSetFiles:
        testingSetTmp.append(transformImageGray(file))
    testingSet = np.asarray(testingSetTmp)
    del testingSetTmp[:]
    ...
    # weight 1
    W1 = tf.Variable(tf.truncated_normal([5, 5, 1, 12], stddev=0.1))
    # bias
    B1 = tf.Variable(tf.ones([12]) / 3)
    # convolutional layer 1
    Y1 = tf.nn.conv2d(X, W1, strides=[1, 1, 1, 1], padding='SAME')
    ...
    for step in range(0,len(trainingSet),batchSize):
        ...
        nextBatch = []
        for image in trainingSet[batchBegin:batchEnd]:
            nextBatch.append(transformImageGray(image))
        batch_X = np.asarray(nextBatch)
        del nextBatch[:]
        batch_Y = np.asarray(trainingLabels[batchBegin:batchEnd])
...
def plotResults(train_a, test_a, train_c, test_c, postfix):
    """
    ...
    plt.grid(True)
    #plt.show()
    plt.savefig("accuracy_%s.png" % (postfix,))
    plt.clf()
    ...
    plt.grid(True)
    #plt.show()
    plt.savefig("loss_%s.png" % (postfix,))

    # save values in csv file
    accuracyFile = open("accuracy_%s.csv" % (postfix,), "w")
    accuracyFile.write("#train, test\n")
    lossFile = open("loss_%s.csv" % (postfix,), "w")
    lossFile.write("#train, test\n")
    for i in range(0,len(train_a)):
        accuracyFile.write("%.5f, %.5f\n" % (train_a[i], test_a[i]))
        lossFile.write("%.5f, %.5f\n" % (train_c[i], test_c[i]))
    accuracyFile.close()
    lossFile.close()
