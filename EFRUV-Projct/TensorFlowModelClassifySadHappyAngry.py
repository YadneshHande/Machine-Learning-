...
def createDataSets(smilePath, nonSmilePath, sadPath, contemptPath, dataSetSize, testingSplit):
    ...
    for root, dirs, files in os.walk(smilePath, True):
        ...
        #all images
        #for name in files:
            if name.endswith(".jpg") and (i<(dataSetSize/4) or dataSetSize == -1):
                if random.randint(1, 100) > testingSplit:
                    trainingSetFiles.append(os.path.join(root, name))
                    trainingLabels.append(np.array([1,0,0,0], np.int32))
                else:
                    testingSet.append(transformImage(os.path.join(root, name)))
                    testingLabels.append(np.array([1,0,0,0], np.int32))
                i=i+1
    ...
    for root, dirs, files in os.walk(nonSmilePath, True):
        ...
        #static for loop
        for name in files:
            if name.endswith(".jpg") and (k<(dataSetSize/4) or dataSetSize == -1):
                if random.randint(1, 100) > testingSplit:
                    ...
                    trainingLabels.insert(insertPosition, np.array([0, 1, 0, 0], np.int32))
                else:
                    # insert to a random position to avoid overfitting
                    insertPosition = random.randint(0, len(trainingLabels))
                    testingSet.insert(insertPosition, transformImage(os.path.join(root, name)))
                    testingLabels.insert(insertPosition, np.array([0, 1, 0, 0], np.int32))
                k=k+1
    
    # transform all sad pictures
    # the sad pictures are added to a random position in the trainingSet and labels and the testingSet and labels
    # the sets and labelled where already created in the above for loop. 
    for root, dirs, files in os.walk(sadPath, True):
        m=0
        #all images
        #for name in files:
        #static for loop
        for name in files:
            if name.endswith(".jpg") and (m<(dataSetSize/4) or dataSetSize == -1):
                if random.randint(1, 100) > testingSplit:
                    # insert to a random position to avoid overfitting
                    insertPosition = random.randint(0, len(trainingLabels))
                    trainingSetFiles.insert(insertPosition, os.path.join(root, name))
                    trainingLabels.insert(insertPosition, np.array([0, 0, 1, 0], np.int32))
                else:
                    # insert to a random position to avoid overfitting
                    insertPosition = random.randint(0, len(trainingLabels))
                    testingSet.insert(insertPosition, transformImage(os.path.join(root, name)))
                    testingLabels.insert(insertPosition, np.array([0, 0, 1, 0], np.int32))
                m=m+1
    
    # transform all contempt pictures
    # the angry pictures are added to a random position in the trainingSet and labels and the testingSet and labels
    # the sets and labelled where already created in the above for loop. 
    for root, dirs, files in os.walk(contemptPath, True):
        n=0
        #all images
        #for name in files:
        #static for loop
        for name in files:
            if name.endswith(".jpg") and (n<(dataSetSize/4) or dataSetSize == -1):
                if random.randint(1, 100) > testingSplit:
                    # insert to a random position to avoid overfitting
                    insertPosition = random.randint(0, len(trainingLabels))
                    trainingSetFiles.insert(insertPosition, os.path.join(root, name))
                    trainingLabels.insert(insertPosition, np.array([0, 0, 0, 1], np.int32))
                else:
                    # insert to a random position to avoid overfitting
                    insertPosition = random.randint(0, len(trainingLabels))
                    testingSet.insert(insertPosition, transformImage(os.path.join(root, name)))
                    testingLabels.insert(insertPosition, np.array([0, 0, 0, 1], np.int32))
                n=n+1
...
def tensorFlowModel(trainingSet,trainingLabels,testingSet,testingLabels,batchSize):
    ...
    # weight 2
    W2 = tf.Variable(tf.truncated_normal([160 * 120 * 12, 4], stddev=0.1))
    # bias
    B2 = tf.Variable(tf.ones([4]) / 3)
...
def main(argv=None):
    ...
    testingSplit = 25 # in % of total data-set size
    batchSize = 25
    trainingSetFiles, trainingLabels, testingSet, testingLabels = createDataSets("AMFED/AMFED/happinessFourteenFramesPerSecond/","AMFED/AMFED/nonHappinessFourteenFramesPerSecond/","AMFED/AMFED/sadnessFourteenFramesPerSecond/","AMFED/AMFED/contemptFourteenFramesPerSecond/",dataSetSize,testingSplit)
